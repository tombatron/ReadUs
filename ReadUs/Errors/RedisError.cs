...
internal sealed class RedisError : IErrorDetails
{
    ...
    public RedisError(IErrorResult? childError, string[] messages, string callerFilePath, int callerLineNumber)
    {
    ...
    public static Result<T> Create<T>(
        ...
        [CallerFilePath] string callerFilePath = "",
        [CallerLineNumber] int callerLineNumber = 0) where T : notnull =>
        Result<T>.Error(new RedisError(childError, messages, callerFilePath, callerLineNumber));
}