...
public class OldRedisConnection : IRedisConnection
{
    ...
    public OldRedisConnection(RedisConnectionConfiguration configuration) :
        this(configuration.ServerAddress, configuration.ServerPort)
    {
    }

    public OldRedisConnection(string address, int port) :
        this(address, port, TimeSpan.FromSeconds(30))
    {
    }

    public OldRedisConnection(string address, int port, TimeSpan commandTimeout) :
        this(ResolveIpAddress(address), port, commandTimeout)
    {
    }

    public OldRedisConnection(IPAddress address, int port) :
        this(address, port, TimeSpan.FromSeconds(30))
    {
    }

    public OldRedisConnection(IPAddress address, int port, TimeSpan commandTimeout) :
        this(new IPEndPoint(address, port), commandTimeout)
    {
    }

    public OldRedisConnection(IPEndPoint endPoint) :
        this(endPoint, TimeSpan.FromSeconds(30))
    {
    }

    public OldRedisConnection(IPEndPoint endPoint, TimeSpan commandTimeout)
    {
        EndPoint = endPoint;
